...
/**
 * GET /api/review/features/:parkingId
 * 주차장 특성 조회 (전기차 충전 등)
 */
router.get('/features/:parkingId', async (req, res) => {
  try {
    const { parkingId } = req.params;
    
    console.log(`📊 주차장 특성 조회: ${parkingId}`);
    ...
    const features = await reviewService.getParkingFeatures(parkingId);
    
    console.log(`✅ 주차장 특성 결과 (${parkingId}):`, {
      hasEVCharging: features.hasEVCharging,
      evChargingCount: features.evChargingReviewCount
    });
    
    res.json({ success: true, data: features });
  } catch (error) {
    console.error(`❌ 주차장 특성 조회 실패 (${req.params.parkingId}):`, error);
    res.status(500).json({ 
      success: false, 
      error: error.message 
    });
...
router.get('/health', (req, res) => {
  res.json({
    ...
    endpoints: {
      ...
      getFeatures: "GET /api/review/features/:parkingId",
