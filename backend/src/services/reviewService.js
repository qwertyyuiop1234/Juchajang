...
export class ReviewService {
  ...
  /**
   * 주차장 특성 정보 조회 (전기차 충전 등)
   */
  async getParkingFeatures(parkingId) {
    try {
      const stats = await this.getReviewStats(parkingId);
      
      return {
        hasEVCharging: (stats.categoryStats.ev_charging || 0) > 0,
        evChargingReviewCount: stats.categoryStats.ev_charging || 0,
        isSpacious: (stats.categoryStats.space_wide || 0) > (stats.categoryStats.space_narrow || 0),
        isPriceFriendly: (stats.categoryStats.price_cheap || 0) > (stats.categoryStats.price_expensive || 0),
        isSafe: (stats.categoryStats.safety_good || 0) > (stats.categoryStats.safety_bad || 0),
        allCategories: stats.categoryStats
      };
    } catch (error) {
      console.error("주차장 특성 조회 실패:", error);
      throw new Error(`주차장 특성 조회 실패: ${error.message}`);
    }
  }
