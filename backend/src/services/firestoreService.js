...
export class FirestoreService {
  ...
  async saveParkingLot(parkingData) {
    try {
      const docRef = db.collection(this.collections.PARKING_LOTS).doc(parkingData.parking_code.toString());
  ...
  async getParkingLot(parkingCode) {
    try {
      const doc = await db.collection(this.collections.PARKING_LOTS).doc(parkingCode.toString()).get();
  ...
  async getLatestParkingStatus(parkingCode) {
    try {
      const snapshot = await db.collection(this.collections.PARKING_STATUS)
  ...
  async getParkingStatusHistory(parkingCode, startDate, endDate) {
    try {
      let query = db.collection(this.collections.PARKING_STATUS)
        .where('parking_code', '==', parkingCode)
        .orderBy('collected_at', 'desc');
    ...
    try {
      ...
      const lotsInRange = allLots.filter(lot => {
        ...
        const distance = this.calculateDistance(
          centerLat, centerLng,
          parseFloat(lot.lat_wgs84), parseFloat(lot.lng_wgs84)
        );
