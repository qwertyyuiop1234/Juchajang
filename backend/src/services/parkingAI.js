...
export function recommendParking(
  ...
  return new Promise((resolve, reject) => {
    ...
    // parking_lots와 parking_status 조인하여 데이터 가져오기
    const query = `
      SELECT DISTINCT
        pl.parking_code,
        pl.parking_name,
      ...
      LEFT JOIN parking_status ps ON pl.parking_code = ps.parking_code
      WHERE pl.lat_wgs84 IS NOT NULL AND pl.lng_wgs84 IS NOT NULL
        AND pl.lat_wgs84 != '' AND pl.lng_wgs84 != ''
      ORDER BY ps.cur_parking_time DESC
    `;
    ...
    db.all(query, [], (err, rows) => {
      ...
      try {
        ...
        rows.forEach((row) => {
          if (
            !parkingMap.has(row.parking_code) ||
            (row.cur_parking_time &&
              (!parkingMap.get(row.parking_code).cur_parking_time ||
                row.cur_parking_time >
                  parkingMap.get(row.parking_code).cur_parking_time))
          ) {
            parkingMap.set(row.parking_code, row);
          }
        });

        const uniqueRows = Array.from(parkingMap.values());

        const recommendations = uniqueRows
          .map((row) => {
        ...
        reject(new Error(`Data processing failed: ${error.message}`));
      }
    });
  });
}
  ...
  return new Promise((resolve, reject) => {
    ...
    const query = `
      SELECT 
        ...
        ps.cur_parking,
        ps.cur_parking_time,
        ps.parking_status_yn,
        ps.parking_status_name
      FROM parking_lots pl
      LEFT JOIN parking_status ps ON pl.parking_code = ps.parking_code
      WHERE pl.parking_code = ?
      ORDER BY ps.cur_parking_time DESC
      LIMIT 1
    `;

    db.get(query, [parkingCode], (err, row) => {
      db.close();
      ...
      const congestionLevel = predictCongestion(row.capacity, row.cur_parking);
      const availableSpaces = Math.max(
        0,
        (row.capacity || 0) - (row.cur_parking || 0)
      );
